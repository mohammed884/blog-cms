...
import {IRegisterBody, ILoginBody, ISendEmailRequest} from "../interfaces/auth";
const register = async (req : Request, res : Response) => {
    try {
        const body: IRegisterBody = req.body;
        await registerSchema.validateAsync(body);
        const isEmailUsedBefore = await User.findOne({email: body.email});
        if (isEmailUsedBefore) 
            return res.status(401).send({success: false, message: "هذا الايميل مستخدم من قبل"});
...
const login = async (req : Request, res : Response) => {
    try {
        ...
        if (! user) 
            return res.status(401).send({succesfull: false, message: "الايميل و الباسوورد لا يتطباقان"});
        ...
        res.status(201).send({succesfull: true, message: "تم تسجيل الدخول بنجاح"});
    } catch (err) {
        console.log(err);
    }
};
const logout = async (req : Request, res : Response) => {
    try {
        ...
        res.status(201).send({succesfull: true, message: "تم تسجيل الخروج بنجاح"});
    } catch (err) {
        console.log(err);
...
const verifyAccount = (req : Request, res : Response) => {
    try {
        ...
        if (! token) 
            return res.send({success: false, message: "لم يتم توفير توكن لتاكيد الحساب"});
